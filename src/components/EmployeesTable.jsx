...
export default function EmployeesTable({ employees , deleteEmployee , isvalid }){
  ...
  {/* BUTTON CONTAINER */}
        <div className="buttonContainer flex justify-between items-center p-4 bg-[var(--text-C)] text-[var(--bg)] rounded-t-lg">
          <h2 className="text-lg font-semibold tracking-wide">Employee Records</h2>

          <button onClick={ ()=> setEditMode((prev) => !prev)}
                  className="bg-[var(--bg)] text-[var(--text-C)] px-4 py-1.5 
                  rounded-md hover:bg-gray-700 transition cursor-pointer"
                  disabled={!isvalid}>
            {editMode ? "Save" : "Edit"}
          </button>
        </div>

        <table className="w-full border-collapse text-sm text-[var(--text-C)]">
  {/* TABLE HEADER */}
          <thead className="bg-[var(--bg)] border-b border-slate-200">
            <td className="px-4 py-2  font-semibold">Del</td>
                  ...
                  <td key={key} className="px-4 py-2 ">
                    {editMode && key !== "id" ? 
                      <input
                        ...
                        className="border border-slate-300 px-2 py-1 rounded-md w-full  focus:outline-none focus:ring-2 focus:ring-[#314879]"></input> 
